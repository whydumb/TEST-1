...
public class VMCReceiver {
    ...
    public void startVMCReceiver() {
        try {
            receiver = new OSCPortIn(39540);
            OSCListener listener = (time, message) -> {
                String address = message.getAddress();
                if (address.equals("/VMC/Ext/Bone/Pos")) {
                    handleVMCBoneData(message);
                } else if (address.startsWith("/VMC/Ext/Blend/")) {
                    handleVMCBlendShape(message);
                }
            };
            receiver.getDispatcher().addListener("/VMC/Ext/Bone/Pos", listener);
            receiver.getDispatcher().addListener("/VMC/Ext/Blend/Val", listener);
            receiver.startListening();
        } catch (Exception e) {
            logger.error("VMC receiver failed", e);
        }
    }
}
